...
class ResponseTest {
    ...
    fun `failed guess`()
    {
        val response = Response("COW", "___", Response.Status.ONGOING, badGuessesAllowed = 10).guess('A')
    ...
    fun `last failed guess loses`() {
        val response = Response("COW", "___", Response.Status.ONGOING, 10, "123456789").guess('A')
        assertThat(response.status).isEqualTo(Response.Status.LOST)
    }

    @Test
    fun `duplicate success letters in word`() {
        val response = Response("MAMA", "____", Response.Status.ONGOING, badGuessesAllowed = 10).guess('A')
    ...
    {
        val response = Response("ABC", "_BC", Response.Status.ONGOING, badGuessesAllowed = 10).guess('A')
    ...
    fun `duplicate successes are idempotent`() {
        val response = Response("ABC", "___", Response.Status.ONGOING, badGuessesAllowed = 10).guess('A')
    ...
    fun `duplicate failures are idempotent`() {
        val response = Response("ABC", "___", Response.Status.ONGOING, badGuessesAllowed = 10).guess('Q')
    ...
    {
        val response = Response("ABC", "AB_", Response.Status.ONGOING, badGuessesAllowed = 10).guess('C')
        assertThat(response.isWin).isTrue()
    }
